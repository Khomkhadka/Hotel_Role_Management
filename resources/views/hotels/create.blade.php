...
<div class="bg-white w-full p-8 rounded-lg shadow-lg">
    ...
    <form action="{{ route('hotels.store') }}" method="POST" class="space-y-4">
        @csrf
        ...
        <div>
         ...
         <!-- comform Password -->
        <div>
            <label class="block text-sm font-medium mb-1">Comform Password</label>
...
{{-- <div class="mt-4">
    <label for="admin_id" class="block text-sm font-medium mb-1">Assign Admin</label>
    <select name="user_id" id="user_id"
        ...
        @foreach ($hotels as $hotel)
            ...
            @if ($hotel->admins) <!-- Check if the hotel has an admin -->
                <option value="{{ $hotel->admins->id ?? '' }}" 
                        {{ old('admin_id', $hotel->admins->id ?? '') == $hotel->admins->id ? 'selected' : '' }}>
                    {{ $hotel->admins->name }}
                </option>
            @else
                <!-- Option for hotels with no admin -->
                <option value="" disabled>No Admin Assigned</option>
            @endif
        @endforeach
    </select>
</div> --}}
