<x-staff-layout>
    ...
    @can('create-customer')
        <a href="{{ route('staff_customers.create') }}" class="bg-slate-700 text-sm rounded-md text-white px-3 py-2">Create Customers</a>
    @endcan
       </div>
    ...
    <div class="w-full max-w-10xl bg-white p-6 rounded-xl shadow-md mt-6">
                        ...
                        @foreach($customers as $customer)
                            <tr class="hover:bg-gray-50 transition">
                                ...
                                <td class="px-4 py-3 space-x-2 flex">
                                  ...
                                  @can('edit-customer')
                                        <a href="{{ route('staff_customers.edit', $customer->id) }}"
                                  ...
                                  @endcan
                                    ...
                                    <form action="{{ route('staff_customers.destroy', $customer->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        @can('delete-customer')
                                            <button type="submit"
