<x-hotel-layout>
    ...
    <a href="{{ route('hotel_bookings.create') }}" class="bg-slate-700 text-sm rounded-md text-white px-3 py-2">Create Booking</a>
       </div>
    </x-slot>
                    ...
                    @foreach($bookings as $booking)
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-4 py-3 space-x-2 flex">
                                <a href="{{ route('hotel_bookings.edit', $booking->id) }}"
                                   class="bg-gray-900 text-white px-3 py-1.5 rounded-md text-xs hover:bg-gray-700 transition">
                                    Edit
                                </a>
    
                                <form action="{{ route('hotel_bookings.destroy', $booking->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
